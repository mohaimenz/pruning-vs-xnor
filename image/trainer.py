...
class Trainer:
    ...
    def Train(self):
        ...
        net = models.Resnet18(self.opt.classes);
        print(net);
        if self.opt.isCuda:
            net.cuda();

        # optimizer = optim.Adam(net.parameters(), lr=self.opt.lr, weight_decay=self.opt.weight_decay);
        optimizer = optim.SGD(net.parameters(), lr=self.opt.lr, momentum=self.opt.momentum, weight_decay=self.opt.weight_decay);
    ...
    def __save_model(self, acc, epochIdx, net):
        if acc > self.bestAcc:
            ...
            fname = "resnet/models/ts/{}{}_a{:.2f}_e{}.pt";
